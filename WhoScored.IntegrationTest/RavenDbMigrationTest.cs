...
namespace WhoScored.IntegrationTest
{
    ...
    ///</summary>
    [TestClass()]
    public class RavenDbMigrationTest
    {
        ...
        public void MigrateWorldDetailsTest_FromXmlToRavenDb()
        {
            ...
            using (var session = RavenConnector.CreateConnection().OpenSession())
            {
                foreach (IWorldDetails country in worldDetailsInput.LeagueList.First().League.Cast<IWorldDetails>().ToList())
                {
                    country.LeagueInWhoScored = false;
